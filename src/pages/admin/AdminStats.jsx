...
import { TrendingUp, TrendingDown, ShoppingBag, Package, Users, Euro } from 'lucide-react'
...
export default function AdminStats() {
  ...
  const stats = useMemo(() => {
    ...
    const monthlyRevenue = Array.from({ length: 6 }, (_, i) => {
      ...
      const total = orders
        .filter(o => { const od = new Date(o.createdAt); return od.getMonth()===m && od.getFullYear()===y && o.status!=='cancelled' })
    ...
    return { revenue, thisRevenue, revTrend, thisMonthOrders, ordersTrend, topProducts, monthlyRevenue, statusCount }
  }, [orders, products])
  ...
  return (
    <div className="stats-page">
      ...
      <div className="stats-kpi-grid">
        ...
        <StatCard label="Pedidos totales"    value={orders.length}                              icon={Package}     color="#5a52c8"/>
        <StatCard label="Ticket medio"       value={orders.length > 0 ? `${(stats.revenue/orders.filter(o=>o.status!=='cancelled').length||1).toFixed(0)} €` : '—'} icon={TrendingUp} color="#c87a2a"/>
      </div>
      ...
      <div className="stats-cols">
        ...
        <div className="stats-panel">
          ...
          {Object.entries(stats.statusCount).map(([status, count]) => (
            <MiniBar key={status} label={STATUS_LABEL[status]||status}
              value={count} max={orders.length} color={STATUS_COLOR[status]||'var(--muted)'}/>
          ))}
      ...
      {/* Low stock warning */}
      {products.filter(p => p.active && p.stock <= 3).length > 0 && (
        <div className="stats-panel stats-alert" style={{marginTop:24}}>
          <h2>⚠ Stock bajo</h2>
          <div className="low-stock-list">
            {products.filter(p => p.active && p.stock <= 3).map(p => (
              <div key={p.id} className="low-stock-row">
                <img src={p.image} alt={p.name}/>
                <span className="low-stock-name">{p.name}</span>
                <span className={`low-stock-qty ${p.stock === 0 ? 'zero' : ''}`}>{p.stock} ud.</span>
              </div>
            ))}
          </div>
        </div>
