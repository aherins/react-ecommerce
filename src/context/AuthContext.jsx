...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    if (!hasSupabase) {
      ...
      if (demo) { try { setUser(JSON.parse(demo)) } catch {} }
      setLoading(false)
      return
    }
  ...
  async function signIn(email, password) {
    if (!hasSupabase) {
      ...
      return { error: { message: `Credenciales incorrectas. Demo: ${DEMO_EMAIL} / ${DEMO_PASSWORD}` } }
    }
  ...
  async function signOut() {
    if (!hasSupabase) { sessionStorage.removeItem('demo_admin'); setUser(null); return }
